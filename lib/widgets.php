...
class first_mag_featured_posts_widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		$meta							 = $instance[ 'meta' ];
		?>
		<p><?php esc_html_e( 'Widget Layout:', 'first-mag' ) ?></p>
		<div style="text-align: center;"><img src="<?php echo get_template_directory_uri() . '/img/feat-cat1.jpg' ?>"></div>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'first-mag' ); ?></label>
			<input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" />
		</p>
		<?php esc_html_e( 'Description', 'first-mag' ); ?>
		<textarea class="widefat" rows="5" cols="20" id="<?php echo $this->get_field_id( 'text' ); ?>" name="<?php echo $this->get_field_name( 'text' ); ?>"><?php echo $text; ?></textarea>
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php esc_html_e( 'Number of posts to display:', 'first-mag' ); ?></label>
			<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'meta' ); ?>"><?php esc_html_e( 'Enable or disable post meta', 'first-mag' ); ?>:</label><br />
			<input type="radio" <?php checked( $meta, 'on' ) ?> id="<?php echo $this->get_field_id( 'meta' ); ?>" name="<?php echo $this->get_field_name( 'meta' ); ?>" value="on"/><?php esc_html_e( 'Enable', 'first-mag' ); ?><br />
			<input type="radio" <?php checked( $meta, 'off' ) ?> id="<?php echo $this->get_field_id( 'meta' ); ?>" name="<?php echo $this->get_field_name( 'meta' ); ?>" value="off"/><?php esc_html_e( 'Disable', 'first-mag' ); ?><br />
		</p>
		<p><input type="radio" <?php checked( $type, 'latest' ) ?> id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>" value="latest"/><?php esc_html_e( 'Show latest Posts', 'first-mag' ); ?><br />
			<input type="radio" <?php checked( $type, 'category' ) ?> id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>" value="category"/><?php esc_html_e( 'Show posts from a category:', 'first-mag' ); ?><br /></p>

		<p>
			<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php esc_html_e( 'Select category', 'first-mag' ); ?>:</label>
	...
	function widget( $args, $instance ) {
		...
		while ( $get_posts->have_posts() ) : $get_posts->the_post();
			...
			<article > 
				<div <?php post_class(); ?>>                            
					<?php if ( has_post_thumbnail() || first_mag_is_preview() ) : ?>                             
		...
		<?php if ( $type == 'category' ) { ?>
			<div class="widget-footer col-md-12 text-center">
				<a class="btn btn-primary btn-md outline" href="<?php echo esc_url( $cat_link ); ?>" >
					<?php esc_html_e( 'All ', 'first-mag' ) ?><?php $categorys = get_category( $category );
					echo esc_attr( $categorys->category_count ); ?><?php esc_html_e( ' Articles', 'first-mag' ) ?>
				</a>
			</div>
		<?php } ?> 
		<!-- </div> -->
